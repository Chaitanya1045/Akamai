# ServiceNow Change Validation — Final Updated Code

This is the corrected implementation based on the current role structure:
...
There is **no `input_validation.yml` dependency** in this implementation.
...
Approval = Approved
    ↓
Planned start/end present
    ↓
Current UTC inside planned window
    ↓
PASS / BLOCK
```

The live ServiceNow path is read-only and uses the AAP-injected environment variables:
...
No ServiceNow credentials are stored in the role, survey, or normal Extra Vars.
...
# Validate the ServiceNow TLS certificate in live mode.
snow_validate_certs: true

# ServiceNow connection timeout in seconds.
snow_api_timeout: 30

# Raw ServiceNow datetime format used for planned-window comparison.
snow_datetime_format: "%Y-%m-%d %H:%M:%S"
```

Do not add ServiceNow credentials here.
...
# 3. `roles/akamai/tasks/main.yml`

Only the ServiceNow validation task needs to be dispatched here for this implementation.
...
`snow_live_attempt.yml` is an internal helper and is called from `snow_validation.yml`; it must not be imported separately from `main.yml`.

---

# 4. `roles/akamai/tasks/snow_live_attempt.yml`
...
#
# Called by snow_validation.yml using snow_live_attempt_plan.
# =============================================================================
  ...
  block:
    ...
    - name: "ServiceNow | Retrieve change request"
      servicenow.itsm.change_request_info:
        ...
        # Return actual and display values.
        # Display values are used for state/approval.
        # Raw values are used for date/time comparison.
    ...
    - name: "ServiceNow | Log failed retrieval attempt"
      ...
      vars:
        ...
        common_logging_message: >-
          ServiceNow retrieval attempt
          {{ snow_live_attempt.attempt }} failed.

        common_logging_recommended_action: >-
          Automatic retry processing will continue when
    ...
    - name: "ServiceNow | Display retry status"
      ansible.builtin.debug:
        msg: >-
          ServiceNow retrieval attempt
          {{ snow_live_attempt.attempt }} of
          {{ snow_live_attempt_plan | length }}
          failed. {{
            'A retry will be attempted.'
            if (
              snow_live_attempt.attempt | int
              <
              snow_live_attempt_plan | length
            )
            else
...
# 5. `roles/akamai/tasks/snow_validation.yml`
...
#
# LIVE:
#   servicenow.itsm.change_request_info -> records[0] -> snow_chg
#
# All business validation below snow_chg is common to both sources.
# =============================================================================

# =============================================================================
# Build common_logging context
# =============================================================================
  ...
  ansible.builtin.set_fact:
    _snow_log_request_id: >-
      {{
        ...
        (
          akamai_chg_number
          | default('SERVICENOW-VALIDATION', true)
          | string
          | trim
        )
      }}
...
# =============================================================================
# Validate local configuration and required CHG input
# =============================================================================

- name: "ServiceNow | Validate execution configuration"
  block:

    - name: "ServiceNow | Validate mode, change number and business gates"
      ansible.builtin.assert:
        that:
          ...
          - >-
            (akamai_chg_number | string | trim)
            is match('^CHG[0-9]+$')

          - >-
            (snow_required_state | string | trim | lower)
            == 'implement'

          - >-
            (snow_required_approval | string | trim | lower)
            == 'approved'
    ...
    - name: "ServiceNow | Stop on invalid configuration"
      ...
      vars:
        ...
        common_logging_error_recommended_action: >-
          Verify the change number, mode, mock scenario
...
- name: "ServiceNow | Simulate mock retry exhaustion"
  ...
  block:
    ...
    - name: "ServiceNow | Log mock retry attempt 1"
      ansible.builtin.debug:
        msg: >-
          ServiceNow mock attempt 1 of 3 failed with
    ...
    - name: "ServiceNow | Log mock retry attempt 2"
      ansible.builtin.debug:
        msg: >-
          ServiceNow mock attempt 2 of 3 failed with
          HTTP {{ _snow_raw.http_status }}.
    ...
    - name: "ServiceNow | Resolve mock terminal failure"
      ansible.builtin.set_fact:
...
- name: "ServiceNow | Handle unexpected mock API failure"
  ...
  vars:
    ...
    common_logging_error_message: >-
      ServiceNow mock retrieval returned an unexpected
      HTTP status.
...
# =============================================================================
# NORMALIZE SUCCESSFUL MOCK RESULT
# =============================================================================

- name: "ServiceNow | Normalize successful mock result"
  when:
    - snow_mode == 'mock'
    - _snow_raw.http_status | int == 200
  block:
    ...
    - name: "ServiceNow | Log successful mock retrieval"
      ...
      vars:
        ...
        common_logging_message: >-
          ServiceNow mock change retrieval completed successfully.
    ...
    - name: "ServiceNow | Stop on invalid mock result"
      ...
      vars:
        ...
        common_logging_error_recommended_action: >-
          Verify the mock JSON response structure.
  ...
  block:
    ...
    - name: "ServiceNow | Check injected credential values"
      ansible.builtin.assert:
        ...
        fail_msg: >-
          Required ServiceNow credential values were not
          injected into the Execution Environment.
...
- name: "ServiceNow | Retrieve change from live API"
  ...
  block:
    ...
    - name: "ServiceNow | Stop after live retrieval failure"
      ...
      vars:
        ...
        common_logging_error_message: >-
          ...
          {{ _snow_live_attempt_used }} attempts.

        common_logging_error_recommended_action: >-
          Verify ServiceNow availability, AAP credential
          configuration, certificate trust, network connectivity,
    ...
    - name: "ServiceNow | Validate live response"
      block:

        - name: "ServiceNow | Validate records collection"
          ansible.builtin.assert:
            that:
            ...
            fail_msg: >-
              ServiceNow did not return a valid records collection.

            quiet: true

        - name: "ServiceNow | Stop when change is not found"
          ansible.builtin.include_role:
            name: common_logging
            tasks_from: fail_with_error
          vars:
        ...
        - name: "ServiceNow | Require exactly one returned change"
          ansible.builtin.assert:
            that:
        ...
        - name: "ServiceNow | Stop on invalid live response"
          ...
          vars:
            ...
            common_logging_error_message: >-
              ServiceNow returned an invalid or ambiguous
              change response.

            common_logging_error_recommended_action: >-
              Verify the change number, ServiceNow response
    ...
    - name: "ServiceNow | Log successful live retrieval"
      ...
      vars:
        ...
        common_logging_message: >-
          ServiceNow change retrieved successfully
          using the read-only information module.
  ...
  block:
    ...
    - name: "ServiceNow | Require mandatory fields"
      ansible.builtin.assert:
        ...
        fail_msg: >-
          ServiceNow change record is missing mandatory fields.
    ...
    - name: "ServiceNow | Stop on missing mandatory fields"
      ...
      vars:
        ...
        common_logging_error_recommended_action: >-
          Verify ServiceNow field availability and
...
#
# Raw values are used for UTC comparison.
# Mock values remain plain strings in the same datetime format.
# =============================================================================
...
# =============================================================================
# Validate returned change identity
# =============================================================================
...
- name: "ServiceNow | Validate returned change identity"
  block:
    ...
    - name: "ServiceNow | Verify change number and sys_id"
      ansible.builtin.assert:
        that:
          - _snow_number == (akamai_chg_number | string | trim)
          - _snow_sys_id | length > 0

        fail_msg: >-
          ServiceNow returned an invalid or unexpected
          change record.
    ...
    - name: "ServiceNow | Stop on invalid change identity"
      ...
      vars:
        ...
        common_logging_error_message: >-
          ServiceNow response does not match the requested
          change number or does not contain a sys_id.
...
# =============================================================================
# Mandatory planned implementation window
# =============================================================================
  ...
  block:
    ...
    - name: "ServiceNow | Require valid window ordering"
      ansible.builtin.assert:
        ...
        fail_msg: >-
          ServiceNow planned start must occur before
          planned end.
    ...
    - name: "ServiceNow | Stop on planned-window failure"
      ...
      vars:
        ...
        common_logging_error_recommended_action: >-
          Verify that the change contains a valid planned
          start and end time and run the automation only
...
Expected: `SERVICENOW_STATUS_INVALID`.
...
Expected: `SERVICENOW_WINDOW_MISSING`.
...
Expected: `SERVICENOW_CHANGE_NOT_FOUND`.
...
Expected: `SERVICENOW_AUTH_FAILED`.
...
Expected: three simulated attempts followed by `SERVICENOW_API_TIMEOUT`.
...
Expected: `SERVICENOW_OUTSIDE_WINDOW`.
...
There is no artificial `akamai_input_validation_passed` variable and no dependency on `input_validation.yml`.
...
Use the collection version approved for the AAP Execution Environment if your organization pins versions.

---

# 9. AAP Credential

The ServiceNow AAP credential must inject:
...
The role explicitly consumes those environment variables in live mode.

Do not pass these values through the survey or Extra Vars.
...
When ServiceNow connectivity and the AAP credential are available:
...
Nothing else related to ServiceNow authentication needs to be supplied in Extra Vars.
...
snow_chg
    ↓
same validation used by mock
    ↓
Implement
AND
Approved
AND
inside planned implementation window
    ↓
PASS / BLOCK
```
...
A successful validation publishes:
...
The validation contract remains the same.
...
Current UTC <= planned end
        ↓
ServiceNow validation PASSED
```

Failure scenarios are routed through the existing `common_logging` role with standardized error categories, keys, messages, and recommended actions.

The live branch uses the read-only `servicenow.itsm.change_request_info` module and is designed so that enabling live ServiceNow changes only the retrieval source; the business-validation logic remains the same.

# SSPA-1 — Project Structure & Variable Reference

**Project:** Enable Application Teams to Self-Serve Akamai Delivery Config Pushes
**Epic:** SSPA-1 | **Stories covered:** SSPA-4, SSPA-8, SSPA-14
...
| Role | Maps To Ticket | Reads or Writes? | Responsibility |
|---|---|---|---|
| `snow_chg_validate` | **SSPA-4 + SSPA-8** | **READ ONLY** | Gate check before deployment: is the CHG approved, in Implement state, and inside its window? |
| `akamai_property` | SSPA-1 (core) | Read + Write (Akamai) | Create version → diff → activate → poll → rollback |
| `snow_ctask_update` | **SSPA-14** | **WRITE** | Append deployment result to the ctask work notes after execution |
...
**Design constraint from SSPA-4 acceptance criteria:** *"No ServiceNow records are modified."* This role only reads. If any check fails, the play stops immediately with a message naming which condition failed and what the actual value was.
...
| `activation_poll_interval` | `10` | `vars.yml` | `poll_status.yml` | Seconds between status checks — matches Confluence spec |
| `activation_poll_max_attempts` | `360` | `vars.yml` | `poll_status.yml` | 360 × 10s = 60-minute timeout — matches Confluence spec |
...
| `jira_ticket` | Never (optional) | `SSPA-42` | Non-prod tracking reference only |
...
## 8. Ticket → Deliverable Traceability

| Ticket | Title | Deliverable in This Project | Status |
|---|---|---|---|
| SSPA-4 | ServiceNow Integration (API & Change Status + Window Check) | `roles/snow_chg_validate/` — query + field extraction | Design complete, blocked on field names |
| SSPA-8 | PROD Change Status Validation (Read-Only) | `roles/snow_chg_validate/` — assertion logic | Design complete, blocked on value formats |
| SSPA-14 | ctask Note Updates with Deployment Details | `roles/snow_ctask_update/` | Design complete, blocked on table name |
| SSPA-1 | Epic: self-serve Akamai config push | Full pipeline: `playbooks/` + `roles/akamai_property/` | Design complete, blocked on Akamai IDs & credentials |

---

## 9. Note for Review

Two of the source tickets have acceptance criteria that don't match the confirmed business rule:

- **SSPA-4** states `"Implement" OR "Approved"` — implies either value of one field is acceptable.
- **SSPA-8** states `Only "Implement" status is accepted` — no mention of approval at all.
- **Confirmed rule:** `state == Implement` **AND** `approval == Approved` — two separate fields, both required.

The implementation follows the confirmed rule. Recommend updating both ticket descriptions so the written acceptance criteria match what was built — on a compliance-facing change gate, a mismatch between documented criteria and implemented logic is the kind of gap that surfaces during audit.
